...
fn main(in: FragmentIn) -> @location(0) vec4f {
    ...
    let imageUV = in.uv * vec2(1,-1 * dimsRatio) * in.ratio.y / params.scale;
    var rgbaImage = texture(image, feedbackSampler, in.uvr, true);
    ...
    let rgbaImageRight = texture(image, feedbackSampler, in.uvr + vec2(texelSize.x, 0), true);
    let bRight = brightness(rgbaImageRight) + (.5 * quant_error);

    let rgbaImageLeft = texture(image, feedbackSampler, in.uvr + vec2(-texelSize.x, 0), true);
    let bLeft = brightness(rgbaImageLeft) + (.5 * quant_error);

    let rgbaImageTop = texture(image, feedbackSampler, in.uvr + vec2(0, -texelSize.y), true);
    let bTop = brightness(rgbaImageTop) + (.5 * quant_error);

    let rgbaImageBottom = texture(image, feedbackSampler, in.uvr + vec2(0, texelSize.y), true);
