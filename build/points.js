...
const defaultStructs = /*wgsl*/`struct ComputeIn{@builtin(global_invocation_id)GID:vec3u,@builtin(workgroup_id)WID:vec3u,@builtin(local_invocation_id)LID:vec3u}struct VertexIn{@location(0)position:vec4f,@location(1)color:vec4f,@location(2)uv:vec2f,@location(3)normal:vec3f,@location(4)id:f32,@location(5)barycentrics:vec3f,@location(6)joint:vec4u,@location(7)weight:vec4f,@builtin(vertex_index)vertexIndex:u32,@builtin(instance_index)instanceIndex:u32}struct FragmentIn{@builtin(position)position:vec4f,@location(0)color:vec4f,@location(1)uv:vec2f,@location(2)ratio:vec2f,@location(3)uvr:vec2f,@location(4)mouse:vec2f,@location(5)normal:vec3f,@interpolate(flat)@location(6)id:f32,@location(7)barycentrics:vec3f,@location(8)world:vec3f,}struct Sound{data:array<f32,2048>,}struct Event{updated:u32,data:array<f32,4>}`;
...
const defaultFunctions = /*wgsl*/`fn defaultVertexBody(position:vec4f,color:vec4f,uv:vec2f,normal:vec3f)->FragmentIn{var result:FragmentIn;let ratio=params.ratios[RENDERPASSINDEX];result.ratio=ratio;result.position=position;result.color=color;result.uv=uv;result.uvr=uv*ratio;result.mouse=params._mouse_normalized;result.normal=normal;return result;}`;
...
class Points {
    ...
    async #setTextureElementImage(name, element, shaderStage = null) {
        const { offsetWidth: width, offsetHeight: height } = element;
        this.#nameExists(this.#textures2d, name);
        if (shaderStage) {
