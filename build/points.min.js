...
`;const grayscale=new RenderPass$1(vert$7,frag$7,null,8,8,1,(points,params)=>{points.setSampler("renderpass_feedbackSampler",null);points.setTexture2d("renderpass_feedbackTexture",true)});const vert$6=`
...
`;const chromaticAberration=new RenderPass$1(vert$6,frag$6,null,8,8,1,(points,params)=>{points.setSampler("renderpass_feedbackSampler",null);points.setTexture2d("renderpass_feedbackTexture",true);points.setUniform("chromaticAberration_distance",params.distance||.02)});chromaticAberration.required=["distance"];const vert$5=`
...
`;const pixelate=new RenderPass$1(vert$5,frag$5,null,8,8,1,(points,params)=>{points.setSampler("renderpass_feedbackSampler",null);points.setTexture2d("renderpass_feedbackTexture",true);points.setUniform("pixelate_pixelDims",params.pixelDimensions||[10,10],"vec2f")});pixelate.required=["pixelDimensions"];const vert$4=`
...
`;const filmgrain=new RenderPass$1(vert$3,frag$3,null,8,8,1,(points,params)=>{points.setSampler("renderpass_feedbackSampler",null);points.setTexture2d("renderpass_feedbackTexture",true)});const vert$2=`
...
`;const bloom=new RenderPass$1(vert$2,frag$2,null,8,8,1,(points,params)=>{points.setSampler("renderpass_feedbackSampler",null);points.setTexture2d("renderpass_feedbackTexture",true);points.setUniform("bloom_amount",params.amount||.5);points.setUniform("bloom_iterations",params.iterations||2)});bloom.required=["amount","iterations"];const vert$1=`
...
`;bindingIndex+=1}}this.#samplers.forEach((sampler,index)=>{if(!sampler.shaderType||sampler.shaderType&shaderType){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${sampler.name}: sampler;
`;bindingIndex+=1}});this.#texturesStorage2d.forEach((texture,index)=>{if(!texture.shaderType||texture.shaderType&shaderType){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${texture.name}: texture_storage_2d<rgba8unorm, write>;
`;bindingIndex+=1}});this.#textures2d.forEach((texture,index)=>{if(!texture.shaderType||texture.shaderType&shaderType){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${texture.name}: texture_2d<f32>;
`;bindingIndex+=1}});this.#textures2dArray.forEach((texture,index)=>{if(!texture.shaderType||texture.shaderType&shaderType){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${texture.name}: texture_2d_array<f32>;
`;bindingIndex+=1}});this.#texturesExternal.forEach(externalTexture=>{if(!externalTexture.shaderType||externalTexture.shaderType&shaderType){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${externalTexture.name}: texture_external;
`;bindingIndex+=1}});this.#bindingTextures.forEach(bindingTexture=>{const{usesRenderPass}=bindingTexture;if(usesRenderPass){if(renderPassIndex===bindingTexture.write.renderPassIndex){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${bindingTexture.write.name}: texture_storage_2d<rgba8unorm, write>;
...
`;bindingIndex+=1}return}if(bindingTexture.write.shaderType&shaderType){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${bindingTexture.write.name}: texture_storage_2d<rgba8unorm, write>;
`;bindingIndex+=1}if(bindingTexture.read.shaderType&shaderType){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${bindingTexture.read.name}: texture_2d<f32>;
