...
`;bindingIndex+=1}}this.#samplers.forEach((sampler,index)=>{const internalCheck=internal==sampler.internal;if(!sampler.shaderType&&internalCheck||sampler.shaderType==shaderType&&internalCheck){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${sampler.name}: sampler;
`;bindingIndex+=1}});this.#texturesStorage2d.forEach((texture,index)=>{const internalCheck=internal&&texture.internal;if(!texture.shaderType&&internalCheck||texture.shaderType==shaderType&&internalCheck){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${texture.name}: texture_storage_2d<rgba8unorm, write>;
`;bindingIndex+=1}});this.#textures2d.forEach((texture,index)=>{const internalCheck=internal==texture.internal;if(!texture.shaderType&&internalCheck||texture.shaderType==shaderType&&internalCheck){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${texture.name}: texture_2d<f32>;
`;bindingIndex+=1}});this.#textures2dArray.forEach((texture,index)=>{const internalCheck=internal==texture.internal;if(!texture.shaderType&&internalCheck||texture.shaderType==shaderType&&internalCheck){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${texture.name}: texture_2d_array<f32>;
`;bindingIndex+=1}});this.#texturesExternal.forEach(externalTexture=>{const internalCheck=internal==externalTexture.internal;if(!externalTexture.shaderType&&internalCheck||externalTexture.shaderType==shaderType&&internalCheck){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${externalTexture.name}: texture_external;
`;bindingIndex+=1}});this.#bindingTextures.forEach(bindingTexture=>{const internalCheck=internal==bindingTexture.internal;if(bindingTexture.compute.shaderType==shaderType&&internalCheck){dynamicGroupBindings+=`@group(${groupId}) @binding(${bindingIndex}) var ${bindingTexture.compute.name}: texture_storage_2d<rgba8unorm, write>;
